...
register("actionbar", (g) => {
    if(config.config_type !== 2) return;
...
register("entityDeath", (entity) => {
    if(config.config_type !== 2) return;
    if(!is_in_mist) return;//*just to be safer
    let distance_to_player = parseInt(entity.distanceTo(Player.getPlayer()));
    if(distance_to_player < 4){
        if(entity.getName() == "Creeper"){
...
register("step", () => {
    if(!config.config_display || config.config_type !== 2) return;
