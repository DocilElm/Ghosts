...
let old_helditem = null;
let sorrow_profit = 0;
let volta_profit = 0;
let plasma_profit = 0;
let is_in_mist = false;
register('actionbar', (g) => {
    if(config.config_type !== 1) return;
    ...
    let h_item = Player.getHeldItem();
    if(h_item == null || h_item.getLore() == null || !World.isLoaded()) return;
    h_item.getLore().map((lore) => {
        let h_name = h_item.getName();
        if(lore.includes("Kills")){
            if(old_helditem == null) old_helditem = h_name;
            let new_kills = parseInt(lore.split(":")[1].replace(",", "").removeFormatting());
            if(old_kills == 0) old_kills = new_kills;
            else if(new_kills !== old_kills && old_helditem == h_name){
...
register("step", () => {
    if(!config.config_display || config.config_type !== 1) return;
